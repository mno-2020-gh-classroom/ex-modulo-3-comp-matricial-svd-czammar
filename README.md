...
Por otro lado, la descomposición SVD de una matriz $A$ consiste en hallar una factorización de ésta en términos de dos matrices ortonormales $U$ y $V$ asociadas a su dominio y co-dominio, junto con una matriz diagonal $\Sigma$ cuyas entradas se encuentran determinadas por los **valores singulares** de $A$. En el contexto de un problema de resolver un sistema $Ax=b$, se puede aprovechar para usar la estructura de las matrices $U$, $V$ y $\Sigma$, para replantear el sistema original, en otro más sencillo que se puede resolver con sustitución hacia atrás/adelante, para posteriormente recuperar la solución de interés con una multiplicación *matriz-vector*.
...
Con este procedimiento reducimos el problema de hallar la solución de un sistema $Ax = b$, pero debemos contar con un método que permita resolver los sistemas lineales más pequeños que se describen en los numerales 1. y 2.
